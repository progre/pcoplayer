...
pub enum BbsUrl {
    ...
    ProbablyShitarabaBoard(Url, String),
...
impl BbsUrl {
    pub fn into_url(self) -> Url {
        match self {
            ...
            BbsUrl::ProbablyShitarabaBoard(url, _) => url,
...
pub fn parse_bbs_url(url: Url) -> Result<BbsUrl, Url> {
    ...
    if let Some(board) = shitaraba::parse_board_url(&url) {
        return Ok(BbsUrl::ProbablyShitarabaBoard(url, board));
    }
...
pub async fn fetch_thread_url_encoding_name(bbs_url: &BbsUrl) -> Result<(Url, String, String)> {
    match bbs_url {
        ...
        BbsUrl::ProbablyShitarabaBoard(url, board) => {
            let origin = url.origin().ascii_serialization();
            let url = shitaraba::fetch_latest_thread_url(&origin, board).await?;
            let (encoding, title) = fetch_charset_title_pair(&url).await?;
